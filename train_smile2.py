...
def vae_loss(args, batch_X, batch_Y_obs, batch_D_score_1, batch_G_A, batch_X_rec, batch_Y_rec, batch_A_rec, batch_idx):
    # for encoder and decoder
    loss_align_1 = loss_an(batch_D_score_1, batch_Y_obs)
...
def train(args):
    ...
    for epoch in range(0, args.ep):
        ...
        for i, (batch_X, batch_Y_obs, batch_Y, batch_idx) in enumerate(train_loader):
            ...
            batch_D_score = args.mu * batch_D_score_1 + (1 - args.mu) * batch_D_score_2
            batch_D = batch_D_score.clone().detach()
            # batch_D = batch_Y_score.clone().detach()
            ...
            gcn_opt.zero_grad()
            loss_distribution, loss_align_1, loss_else = vae_loss(args, batch_X, batch_Y_obs, batch_D_score_1, batch_G_A, batch_X_rec, batch_Y_rec, batch_A_rec, batch_idx)
            loss_distribution.backward()
            gcn_opt.step()
        # evaluate
